    ...
    _resolveSprite(tile, entity, season) {
        ...
        if (entity) {
            if (entity.type === 'colonist') {
                ...
                const variant = sm.resolveColonistVariant(entity.colonistId, entity.skinSeed, entity.skinVariants);
                if (entity.armorKey || entity.helmetKey || entity.weaponKey || entity.toolKey) {
                    const comp = sm.getCompositedColonistSprite(entity.colonistId, entity.drafted, entity.armorKey, entity.helmetKey, entity.gender, entity.weaponKey, entity.toolKey, variant);
                    if (comp) return comp;
                }
                return sm.getColonistSprite(entity.colonistId, entity.drafted, entity.gender, variant);
            }
    ...
    render(game) {
        ...
        for (const c of colonists) {
            if (c.hp > 0 && !c.onExpedition) {
                ...
                const entData = { char: c.golem ? 'G' : '@', color, type: c.golem ? 'golem' : 'colonist', colonistId: c.id, gender: c.gender, skinSeed: c.skinSeed, skinVariants: c.skinVariants, drafted, golemType: c.golemType, sleeping: isSleeping, sleepingInBed, _dmgFlashUntil: c._dmgFlashUntil, _atkShakeUntil: c._atkShakeUntil, armorKey: showEq ? (c.armor?.key || null) : null, helmetKey: showEq ? (c.helmet?.key || null) : null, weaponKey: showEq ? (c.weapon?.key || null) : null, toolKey: showEq ? (c.tool?.key || null) : null };
                if (isEntityMoving(c)) {
